...
// Shared renderer state. The only platform difference is how the window/surface
// handle is obtained, which is handled by the platform modules below.
pub struct SaverState {
    ...
    frames: i32,
    pipeline: crate::pipeline::MatrixPipeline,
}
...
impl SaverState {
    pub fn apply_settings(
        ...
        let mut pipeline = crate::pipeline::MatrixPipeline::new(
            &self.device,
            &self.queue,
            &self.time_buffer,
            config,
    ...
    pub fn render(&mut self) {
        ...
        self.queue
            .write_buffer(&self.time_buffer, 0, &crate::gpu::uniform_bytes(&time));
...
// Builds a `SaverState` from a wgpu surface target. Shared by both platforms.
pub fn build_state<W: raw_window_handle::HasWindowHandle + raw_window_handle::HasDisplayHandle>(
    window: &W,
    width: u32,
    height: u32,
    version: &str,
    ...
    let time_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        ...
        size: crate::gpu::uniform_size::<TimeUniform>(),
    ...
    pipeline.build(&device, &queue, [surface_config.width, surface_config.height]);
...
mod macos {
    ...
    use super::{build_state, SaverState};
    ...
    pub extern "C" fn matrix_saver_new(
        ...
        let ns_view = match NonNull::new(ns_view) {
            Some(view) => view,
            None => {
                eprintln!("failed to create Tank saver renderer: null NSView");
...
mod windows {
    ...
    impl HasWindowHandle for HwndHandle {
        fn window_handle(&self) -> Result<WindowHandle<'_>, HandleError> {
            let mut handle = Win32WindowHandle::new(
                std::num::NonZeroIsize::new(self.hwnd as isize).ok_or(HandleError::NotSupported)?,
            );
            handle.hinstance = None;
            // SAFETY: the pointer is a valid HWND owned by the caller for the lifetime
            // of the surface; wgpu only reads it during surface creation.
    ...
    pub extern "C" fn matrix_saver_new(
        ...
        let version = unsafe { version_from_ptr(version) };
        if hwnd.is_null() {
            eprintln!("failed to create Tank saver renderer: null HWND");
            return std::ptr::null_mut();
        }
        let hwnd_handle = HwndHandle { hwnd };
        match build_state(
            &hwnd_handle,
            width,
            height,
